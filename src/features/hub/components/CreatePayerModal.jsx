...
export default function CreatePayerContractModal({ show, onClose }) {
  ...
  // arrays
  const [phones, setPhones] = useState([]);     // {label, number, notes}
  const [portals, setPortals] = useState([]);   // {label, url, notes}
  const [addresses, setAddresses] = useState([]); // {label,line1,line2,city,state,zip,notes}
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await axios.post(`${API}/hub/payer-contracts`, {
    ...
    <div onClick={(e) => { if (e.target === e.currentTarget) onClose?.(); }}
      ...
      <div className="card shadow-lg" style={{ width: 'min(980px, 96vw)' }}>
        <div className="card-header d-flex justify-content-between align-items-center">
          <h5 className="mb-0">Create Contract</h5>
