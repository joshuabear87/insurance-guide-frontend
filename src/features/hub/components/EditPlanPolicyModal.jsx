...
export default function EditPlanPolicyModal({ show, policy, onClose }) {
  ...
  useEffect(() => {
    ...
    setFacilityName(policy?.facilityName || '');
    setNetworkStatus(policy?.networkStatus || NETWORK_STATUSES[0]);
    setNotes(policy?.notes || '');
    setPhones(Array.isArray(policy?.phones) ? policy.phones : []);
    setPortals(Array.isArray(policy?.portals) ? policy.portals : []);
    setAddresses(Array.isArray(policy?.addresses) ? policy.addresses : []);
    setSlps(Array.isArray(policy?.serviceLocationPolicies) ? policy.serviceLocationPolicies : []);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await axios.patch(`${API}/hub/plan-policies/${policy._id}`, {
      ...
      enqueueSnackbar('Plan policy updated', { variant: 'success' });
      onClose?.(true);
    } catch (e) {
  ...
  const handleDelete = async () => {
    if (!window.confirm('Delete this plan policy?')) return;
    setDeleting(true);
    try {
      await axios.delete(`${API}/hub/plan-policies/${policy._id}`);
      enqueueSnackbar('Plan policy deleted', { variant: 'info' });
      onClose?.(true);
    } catch (e) {
    ...
    <div onClick={(e) => { if (e.target === e.currentTarget) onClose?.(); }}
      ...
      <div className="card shadow-lg" style={{ width:'min(1100px,96vw)', maxHeight:'92vh', overflow:'hidden' }}>
        <div className="card-header d-flex justify-content-between align-items-center">
          <h5 className="mb-0">Edit Plan Policy</h5>
                ...
                <tbody>
                  {slps.map((s, i) => (
                    <tr key={s._id || i}>
                      ...
                      <td>
                        <select className="form-select" value={s.decision || 'Accept'} onChange={e=>changeSlp(i,'decision',e.target.value)}>
